...
def verify_remote_tag(
    ...
    isolated_ref = f"refs/release-tags/{tag}"
    main_ref = f"refs/remotes/{remote}/main"
    for ref in (remote_ref, isolated_ref, main_ref):
    ...
    git(
        ...
        "--",
        remote,
        f"{remote_ref}:{isolated_ref}",
        f"+refs/heads/main:{main_ref}",
        root=root,
        token=token,
    )
    object_type = git("cat-file", "-t", isolated_ref, root=root)
    if object_type != "tag":
        raise ReleaseTagError(
            f"{remote_ref} is {object_type}, not an annotated tag object"
        )
    tag_object = git("rev-parse", isolated_ref, root=root)
    candidate_revision = git("rev-parse", f"{isolated_ref}^{{commit}}", root=root)
    ...
    return ReleaseTag(isolated_ref, tag_object, candidate_revision, main_revision)
