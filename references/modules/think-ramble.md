...
2. Hand off to the multi-harness substrate contract applied to the skill named `atlas`, loading its `remember` path:
   - Target: subject Atlas root (`<subject>/references/atlas/`; when subject is autogenesis: `references/atlas` after mount) under `autogenesis/experiences/` (or appropriate type).
